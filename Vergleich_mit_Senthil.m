...
%
disp(['Our model:       Recovery = ',num2str(output(4)),' %    C_Permeate = ',num2str(output(9)),' ppm '])
%
disp(' ')
disp(['We fix now:      J_d(0) = ',num2str(131.7146216211166*output(6)),' kg/sm']) % 0.080316925377597
disp(' ')

%% same flow rate, but other inlet pressure
output=fun_unscaled([0.080316925377597; 55.81; 0],0,'sol',1e4,1e-3);
disp('Same flow rate and P_d(0) = 55.81 bar ')
disp(' ')
disp('Experiment:      Recovery = 39.6 %       C_Permeate = 49 ppm')
%
disp(['Our model:       Recovery = ',num2str(output(4)),' %    C_Permeate = ',num2str(output(9)),' ppm '])
%
disp(' ')
output=fun_unscaled([0.080316925377597; 60.28; 0],0,'sol',1e4,1e-3);
disp('Same flow rate and P_d(0) = 60.28 bar ')
disp(' ')
disp('Experiment:      Recovery = 44.5 %       C_Permeate = 52 ppm')
%
disp(['Our model:       Recovery = ',num2str(output(4)),' %    C_Permeate = ',num2str(output(9)),' ppm '])
%
disp(' ')
...
for x=1:length(X)
    out=fun_unscaled([0.080316925377597; X(x); 0],0,'sol',1e4,1e-3);
...
%% save figure
figure(1); set(gcf,'color','w'); f = gcf; exportgraphics(f,'Senthil_vergleich.png');

%% load/save data for plot
%load("Output_DATA/Senthil_verleich_adapted.mat")
%save Output_DATA/Senthil_verleich_adapted.mat X Y Z XX YY ZZ YYY ZZZ
