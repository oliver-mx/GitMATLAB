...
% version(6) = 4 --> SWRO-PRO hybrid system (with two ERDs) %still in progress
...
swro_KD = 1e2;   % SWRO ECP draw side mass transfer coefficient
swro_KF = 1e2;   % SWRO ECP fresh side mass transfer coefficient
...
swro_beta_fix=4.34e-4/J_r*swro_x_r; % value for fixed SWRO beta [kg/sm^2]
...
R =0.94; %0.96;         % salt rejection rate [1]
KK=71.3;                % mass transfer coefficient [sm^2/kg]
KD=71.3;                % PRO ECP draw side mass transfer coefficient [sm^2/kg]
KF=71.3;                % PRO ECP fresh side mass transfer coefficient [sm^2/kg]
...
Pd_L =  input1(2);       % pressure draw side at L (not need if coupled via ERD)
...
pd_L = 13.9e5/p_r;     % pressure of fresh side at 0
pf_0 = 1.5e5/p_r;      % pressure draw side at L 
...
%% display figures
fig=[0,0,0,0,0,0,0,0,0]; % f(i)=1 --> figure i will be displayed
